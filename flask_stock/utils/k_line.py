...
class KLine:
    ...
    def fetch_data(self):
        ...
        self.data = ticker.history(start=start_date, end=end_date, interval='1mo')
        self.save_json()

    def save_json(self):
        filename = f'kline_{self.symbol}.json'
        with open(filename, 'w') as f:
            json.dump(self.data.to_dict('records'), f, indent=4, default=str)

    def process_data(self):
        self.filtered_dates = self.data.index.tolist()
        self.filtered_prices = self.data['Close'].tolist()

    def get_inflection_points(self):
        self.fetch_data()
        self.process_data()
        window_size = 3
        self.inflection_points = self.find_significant_inflections(
            self.filtered_prices,
            window=window_size,
            threshold=self.significant_change_threshold
        )
        return self.format_inflection_points()

    def format_inflection_points(self):
        inflection_data = []
